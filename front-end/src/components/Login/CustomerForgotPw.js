    ...
    const resetPassword = async(event) => {
        ...
        if (password1 == password2) {
            ...
            if (isValidPassword.result) {
                var {data} = await axios.post('http://localhost:8000/general/sms/password_reset_customer', {otp, email, password:password1})
                ...
                if (data.success) {
                    ...
                    navigate(`/user/${data.data[0].cus_id}`)
                }else{
                    setMsg(data.msg)
                }
            }else{
...
export default CustomerForgotPw